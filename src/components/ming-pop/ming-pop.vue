<template>
  ...
  <view :class="direction==='center'?'centers':''" v-if="direction==='center'?open:true">
		<view class="product-window" :style="{width:width+'%'}" :class="(open ? 'on' : '')+' '+direction">
			...
			<!-- #endif -->
			<image src="../../static/ming-pop/close.png" mode="" :class="(direction!=='below'&&direction!=='center')?'iconfont-h5':''" class="iconfont" @click="close" v-if="is_close"></image>
			<slot></slot>
		</view>
	  <!--增加遮罩层能否关闭-->
		<view class="mask" v-if="is_mask && mask_close" @touchmove.prevent :hidden="!open" @click="open=false"></view>
		<view class="mask" v-if="is_mask && !mask_close" @touchmove.prevent :hidden="!open" ></view>
	</view>
</template>
